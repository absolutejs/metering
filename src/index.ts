...
export type MeterEvent = HandlerMeterEvent | ProcessMeterEvent | ObservationMeterEvent;
...
const evictExpired = (state: RollingState, now: number, windowMs: number): number => {
...
export type Meter = {
	 ...
	 */
	rollingSum: (tenant: string, dimension: RollingDimension, windowMs: number) => number;
...
export const createMeter = (options: MeterOptions = {}): Meter => {
	...
	const rollingKey = (tenant: string, dimension: RollingDimension, windowMs: number) =>
		`${tenant}|${dimension}|${windowMs}`;

	const ensureRolling = (tenant: string, dimension: RollingDimension, windowMs: number): RollingState => {
	...
	const recordRolling = (
		...
		if (delta <= 0) return;
		const rules = rollingFor(tenant).filter((rule) => rule.dimension === dimension);
	...
	const isStillTripped = (tenant: string): boolean => {
		...
		for (const rule of rollingFor(tenant)) {
			const state = rollingState.get(rollingKey(tenant, rule.dimension, rule.windowMs));
			if (!state) continue;
	...
	return {
		...
		restore: (snapshot) => {
			...
			for (const entry of snapshot.tenants) {
				usageMap.set(entry.tenant, { ...entry.usage });
				...
				for (const rule of entry.rolling) {
					...
					rollingState.set(rollingKey(entry.tenant, rule.dimension, rule.windowMs), state);
				}
			}
		},
