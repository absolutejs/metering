...
describe('observation events (runtime 0.1.0 shape)', () => {
	test('first observation establishes baseline; subsequent ones charge the delta', () => {
		...
		meter.record({ at: 1000, cpuMs: 100, rssBytes: 50_000_000, tenant: 't1', type: 'observation' });
		expect(meter.usage('t1')!.processCpuMs).toBe(100);

		meter.record({ at: 2000, cpuMs: 250, rssBytes: 80_000_000, tenant: 't1', type: 'observation' });
		expect(meter.usage('t1')!.processCpuMs).toBe(250); // 100 baseline + 150 delta

		meter.record({ at: 3000, cpuMs: 1000, rssBytes: 80_000_000, tenant: 't1', type: 'observation' });
	...
	test('process spawn resets the cumulative-cpu baseline', () => {
		const clock = tick();
		const meter = createMeter({ clock });
		meter.record({ at: 1000, cpuMs: 500, rssBytes: 100_000_000, tenant: 't1', type: 'observation' });
		meter.record({ at: 1500, cpuMs: 800, rssBytes: 100_000_000, tenant: 't1', type: 'observation' });
		expect(meter.usage('t1')!.processCpuMs).toBe(800);

		// Tenant process exited + new one spawned.
		meter.record({ at: 2000, tenant: 't1', transition: 'exit', type: 'process' });
		meter.record({ at: 2100, tenant: 't1', transition: 'spawn', type: 'process' });
		// Fresh observation: 200ms cpu since the new spawn — should add 200, not double-count.
		meter.record({ at: 2500, cpuMs: 200, rssBytes: 60_000_000, tenant: 't1', type: 'observation' });
		expect(meter.usage('t1')!.processCpuMs).toBe(800 + 200);
	});

	test('processRssBytesPeak tracks the high-water mark', () => {
		const meter = createMeter({ clock: tick() });
		meter.record({ at: 1, cpuMs: 0, rssBytes: 100_000_000, tenant: 't1', type: 'observation' });
		meter.record({ at: 2, cpuMs: 5, rssBytes: 400_000_000, tenant: 't1', type: 'observation' });
		meter.record({ at: 3, cpuMs: 10, rssBytes: 200_000_000, tenant: 't1', type: 'observation' });
	...
	test('processCpuMs is a separately-billable cumulative budget dimension', () => {
		...
		const meter = createMeter({
			...
			clock: tick(),
			onBreach: (event) => { breaches.push(event); },
		});
		meter.record({ at: 1, cpuMs: 500, rssBytes: 0, tenant: 't1', type: 'observation' });
		expect(meter.allow('t1')).toBe(true);
		meter.record({ at: 2, cpuMs: 1500, rssBytes: 0, tenant: 't1', type: 'observation' });
		expect(meter.allow('t1')).toBe(false);
...
describe('rolling-window budgets', () => {
	test('trips when rolling sum crosses the limit, re-closes as events fall out', () => {
		...
		const meter = createMeter({
			clock: () => now,
			rollingBudgets: { t1: [{ dimension: 'errors', limit: 3, windowMs: 1000 }] },
		});
		const err = (): MeterEvent => ({ at: now, cpuMs: 1, durationMs: 1, ok: false, tenant: 't1', type: 'handler' });
	...
	test('default wildcard applies when no per-tenant rolling rule exists', () => {
		...
		const meter = createMeter({
			clock: () => now,
			rollingBudgets: { '*': [{ dimension: 'requests', limit: 2, windowMs: 5000 }] },
		});
		const req = (): MeterEvent => ({ at: now, cpuMs: 0, durationMs: 0, ok: true, tenant: 'newcomer', type: 'handler' });
		meter.record(req());
		expect(meter.allow('newcomer')).toBe(true);
		meter.record(req());
		expect(meter.allow('newcomer')).toBe(false);
	...
	test('rolling cpuMs aggregates per-handler cpu', () => {
		...
		const meter = createMeter({
			clock: () => now,
			rollingBudgets: { t1: [{ dimension: 'cpuMs', limit: 500, windowMs: 60_000 }] },
		});
		const h = (cpu: number): MeterEvent => ({ at: now, cpuMs: cpu, durationMs: cpu, ok: true, tenant: 't1', type: 'handler' });
		meter.record(h(200));
		meter.record(h(200));
		expect(meter.allow('t1')).toBe(true);
		meter.record(h(150));
...
describe('flushable sinks', () => {
	test('object-shaped sink with ingest receives every event', () => {
		...
		const meter = createMeter({
			clock: tick(),
			sinks: [{ ingest: (event) => { seen.push(event); } }],
		});
		meter.record({ at: 1, cpuMs: 1, durationMs: 1, ok: true, tenant: 't1', type: 'handler' });
		expect(seen).toHaveLength(1);
	});
	...
	test('dispose awaits flush then close in order, per sink', async () => {
		...
		const meter = createMeter({
			...
			sinks: [
				{
					close: async () => { await new Promise((resolve) => setTimeout(resolve, 5)); order.push('a:close'); },
					flush: async () => { await new Promise((resolve) => setTimeout(resolve, 5)); order.push('a:flush'); },
					ingest: () => {},
				},
				{
					close: async () => { order.push('b:close'); },
					flush: async () => { order.push('b:flush'); },
					ingest: () => {},
				},
			],
		});
	...
	test('a throwing flush does not stop later sinks from flushing', async () => {
		...
		const meter = createMeter({
			clock: tick(),
			sinks: [
				{ flush: async () => { throw new Error('boom'); }, ingest: () => {} },
				{ close: async () => { closed.push('b'); }, flush: async () => { closed.push('b-flushed'); }, ingest: () => {} },
			],
		});
		await meter.dispose();
	...
	test('function-shaped sinks still work alongside object-shaped ones', () => {
		...
		const meter = createMeter({
			clock: tick(),
			sinks: [
				(event) => { fnSeen.push(event); },
				{ ingest: (event) => { objSeen.push(event); } },
			],
		});
		meter.record({ at: 1, cpuMs: 1, durationMs: 1, ok: true, tenant: 't1', type: 'handler' });
...
describe('snapshot + restore', () => {
	test('snapshot serializes; a fresh meter restored from it picks up where the old one left off', () => {
		...
		const meter = createMeter({
			...
			clock: () => now,
			rollingBudgets: { t1: [{ dimension: 'errors', limit: 5, windowMs: 60_000 }] },
		});
		meter.record({ at: now, cpuMs: 5, durationMs: 10, ok: true, tenant: 't1', type: 'handler' });
		meter.record({ at: now, cpuMs: 7, durationMs: 12, ok: false, tenant: 't1', type: 'handler' });
		meter.record({ at: now, cpuMs: 200, rssBytes: 100_000_000, tenant: 't1', type: 'observation' });
		...
		next.record({ at: now + 100, cpuMs: 500, rssBytes: 110_000_000, tenant: 't1', type: 'observation' });
		expect(next.usage('t1')!.processCpuMs).toBe(500);
	});

	test('restore replaces all current state', () => {
		const meter = createMeter({ clock: tick() });
		meter.record({ at: 1, cpuMs: 1, durationMs: 1, ok: true, tenant: 'old-tenant', type: 'handler' });
...
describe('breaker re-closes on rolling-budget drain', () => {
	test('a cumulative trip stays tripped; a rolling-only trip auto-clears', () => {
		...
		const meter = createMeter({
			...
			clock: () => now,
			rollingBudgets: { rollingtenant: [{ dimension: 'requests', limit: 2, windowMs: 1000 }] },
		});
		const req = (tenant: string): MeterEvent => ({ at: now, cpuMs: 0, durationMs: 0, ok: true, tenant, type: 'handler' });
